...
class SoftmaxApprSamplerUniform(SamplerBase):
    ...
    def __init__(self, item_embs, K_or_center, item_pop = None):
        super(SoftmaxApprSamplerUniform, self).__init__(item_embs.shape[0])
        if isinstance(K_or_center, int) is None:
            self.K = K_or_center
        else:
            self.K = K_or_center.size(0)
        #embs1, embs2 = np.array_split(item_embs, 2, axis=-1)
        embs1, embs2 = torch.chunk(item_embs, 2, dim=-1)
        self.c0, cd0, cd0m, _ = kmeans(embs1, K_or_center)
        self.c1, cd1, cd1m, _ = kmeans(embs2, K_or_center)
...
class SoftmaxApprSamplerPop(SoftmaxApprSamplerUniform):
    ...
    def __init__(self, item_embs, pop_count, num_cluster, mode=1):
        if mode == 0:
            ...
            pop_count = pop_count**0.75
        pop_count = torch.from_numpy(pop_count)
        super(SoftmaxApprSamplerPop, self).__init__(item_embs, num_cluster, pop_count)
        #self.p = torch.from_numpy(np.insert(pop_count, 0, 1.0)) # this is similar, to avoid log 0 !!! in case of zero padding 
        self.p = torch.cat([torch.tensor(1.0), pop_count], dim=0) # this is similar, to avoid log 0 !!! in case of zero padding 
