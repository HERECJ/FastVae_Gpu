...
from sampler_gpu import SamplerBase, PopularSampler, MidxUniform, MidxUniPop
import torch
import torch.optim
...
def train_model(model, train_mat, test_mat, config, logger):
    # sampler_list = [SamplerBase, PopularSampler, MidxUniform, MidxUniPop]
    ...
    orig_item_num = train_mat.shape[1]
    initial_list = []
    training_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('-data', default='amazoni', type=str, help='path of datafile')
    ...
    alg = config.model
    sampler = str(config.sampler) + '_' + str(config.multi)
    ...
    logger.info('Running Time: {}'.format(t1-t0) )
